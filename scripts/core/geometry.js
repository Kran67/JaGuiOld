...
class Point extends BaseClass {
    ...
    scale(x, y) {
        ...
        !core.tools.isNumber(x) && (x = 0);
        !core.tools.isNumber(y) && (y = 0);
        core.private(this, { x: priv.x * x, y: priv.y * y });
        return this;
    }
    //#endregion scale
    //#region inRect
...
class Rect extends BaseClass {
    ...
    includedRect(r) {
        ...
        if (r instanceof core.classes.Rect) {
            core.private(this, {
                left: Math.max(priv.left, r.left),
                top: Math.max(priv.top, r.top),
                right: Math.max(priv.right, r.right),
                bottom: Math.max(priv.bottom, r.bottom)
            });
        }
        return this;
    }
