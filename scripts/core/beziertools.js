...
class BezierTools {
    ...
    static arcAsBezier(alpha) {
        ...
        const p2 = new Core.classes.Point(cosA + 4 / 3 * (1 - cosA), sinA - 4 / 3 * cosA * (1 - cosA) / sinA);
        return {
            s: new Core.classes.Point(cosa, -sinA),
            c1: new Core.classes.Point(p2.x, -p2.y),
            c2: p2,
            e: new Core.classes.Point(cosa, sinA)
        };
    }
    /**
